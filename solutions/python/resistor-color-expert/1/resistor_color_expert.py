...
def resistor_label(colors):
    ...
    if len(colors) == 4:  # 4 bandas: 2 dígitos + multiplicador + tolerancia
        numero_str = ''.join(str(colores.index(c)) for c in colors[:2])
        multiplicador = colores.index(colors[2])
    elif len(colors) == 5:  # 5 bandas: 3 dígitos + multiplicador + tolerancia
        numero_str = ''.join(str(colores.index(c)) for c in colors[:3])
    ...
    if valor < 1_000:
        ...
        num_formateado = f"{valor / 1_000_000:.2f}".rstrip('0').rstrip('.')
        return f"{num_formateado} megaohms{tol}"
    else:
        num_formateado = f"{valor / 1_000_000_000:.2f}".rstrip('0').rstrip('.')
        return f"{num_formateado} gigaohms{tol}"
